...
{
	// filter radius to not exceed VGA resolution
	if (r > 230)
		r = 230;
...
void FrameCore::drawRectangleOutline(int x0, int y0, int width, int height, int color)
{
   const auto x1 = x0 + width;
   const auto y1 = y0 + height;
   plot_line(x0, y0, x1, y0, color);
   plot_line(x1, y0, x1, y1, color);
   plot_line(x1, y1, x0, y1, color);
   plot_line(x0, y0, x0, y1, color);
}

void FrameCore::drawRectangle(int x0, int y0, int width, int height, int color)
{
   const auto x1 = x0 + width;
   const auto y1 = y0 + height;
   for(int x = x0; x < x1; x++)
